...
describe('ProjectService', () => {
  ...
  describe('loadProjects', () => {
    ...
    it('should throw error if projects array is empty', async () => {
      ...
      vi.spyOn(fs, 'readFile').mockResolvedValueOnce(JSON.stringify(emptyProjects));
    ...
    it('should throw error if file read fails', async () => {
      ...
      // Verify it throws the expected error
      await expect(ProjectService.loadProjects()).rejects.toThrow('File not found');
    });
    ...
    it('should log error to console when exception occurs', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error loading projects:', fileError);
    });
  });
});