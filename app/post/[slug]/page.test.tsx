...
vi.mock('@/app/components/blog/PostDate', () => ({
  default: ({ created, lastUpdated }: { created: string; lastUpdated: string }) => (
    <span data-testid="post-date">{created} / {lastUpdated}</span>
  ),
}));
...
describe('Post page', () => {
  ...
  it('renders post details and content', async () => {
    ...
    // Publish date
    expect(screen.getByTestId('post-date')).toHaveTextContent('2023-01-01 / 2023-01-02');

    // Markdown content
    expect(screen.getByTestId('markdown')).toHaveTextContent('Test Content');

    // Share buttons
    expect(screen.getByTestId('share-buttons')).toHaveTextContent('Share: Test Post');
  });
});
