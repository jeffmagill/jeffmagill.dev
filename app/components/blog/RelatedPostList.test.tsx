...
describe('RelatedPostList', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(postService.getSlugs).mockImplementation(() => posts.map((p) => p.slug));
    vi.mocked(postService.getPost).mockImplementation((slug) => posts.find((p) => p.slug === slug));
  });
  ...
  test('filters out currentSlug and scores by shared tags and respects maxResults', () => {
    render(<RelatedPostList tags={'tag1,tag2'} currentSlug={'a'} maxResults={2} />);
