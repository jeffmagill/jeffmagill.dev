...
vi.mock('../global/ErrorDisplay', () => ({
  default: ({ title, message, details }: { title: string; message: string; details?: string }) => (
  ...
  default: ({ projects }: { projects: any[] }) => (
    <div data-testid="project-list">
      {projects.map((project, index) => (
        <div key={index} data-testid="project">{project.title}</div>
      ))}
    </div>
  )
}));
...
describe('ProjectListContainer', () => {
  ...
  it('renders error message when service throws an error', async () => {
    ...
    expect(await screen.findByText('There was a problem loading projects.')).toBeInTheDocument();
