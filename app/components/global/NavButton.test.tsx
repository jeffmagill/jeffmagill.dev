...
vi.mock('@fortawesome/react-fontawesome', () => ({
  FontAwesomeIcon: ({ icon }) => <span data-testid="font-awesome-icon">Icon Mock</span>
}));
...
describe('NavButton', () => {
  ...
  it('adds and removes event listeners on mount and unmount', () => {
    ...
    expect(mockHeaderElement.addEventListener).toHaveBeenCalledWith('click', expect.any(Function));
    
    // Unmount component
    unmount();
    
    // Check if event listener was removed on unmount
    expect(mockHeaderElement.removeEventListener).toHaveBeenCalledWith('click', expect.any(Function));
  });
  ...
  it('handles path change and resets checkbox', () => {
    ...
    expect(mockHeaderElement.style.setProperty).toHaveBeenCalledWith('--header-space', '');
  });
});